...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			...
			alert("上传的图片太多了.\n" + (message === 0 ? "已达到图片数量限制." : "你最多可以选择 " + (message > 1 ? "" + message + " 文件." : "1个文件.")));
			return;
		}
...
//			this.debug("Error Code: File too big, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			...
			this.debug("错误码: 文件太大了, 文件名: " + file.name + ", 文件大小: " + file.size + ", 信息: " + message);
			break;
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
//			progress.setStatus("Cannot upload Zero Byte files.");
//			this.debug("Error Code: Zero byte file, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			progress.setStatus("不能上传0M图片.");
			this.debug("错误码: 0M图片, 文件名: " + file.name + ", 文件大小: " + file.size + ", 信息: " + message);
			break;
...
//			this.debug("Error Code: IO Error, File name: " + file.name + ", Message: " + message);
			progress.setStatus("服务器写入错误");
			this.debug("错误码: 服务器写入错误, 文件名: " + file.name + ", 信息: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
//			progress.setStatus("Security Error");
//			this.debug("Error Code: Security Error, File name: " + file.name + ", Message: " + message);
			progress.setStatus("安全性错误");
			this.debug("错误码: 安全性错误, 上传超过限制: " + file.name + ", 信息: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
//			progress.setStatus("Upload limit exceeded.");
//			this.debug("Error Code: Upload Limit Exceeded, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			progress.setStatus("上传超过限制.");
			this.debug("错误码: 上传超过限制, 文件名: " + file.name + ", 文件名: " + file.size + ", 信息: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
//			progress.setStatus("Failed Validation.  Upload skipped.");
//			this.debug("Error Code: File Validation Failed, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			progress.setStatus("验证失败.跳过上传.");
			this.debug("错误码: 验证失败, 文件名: " + file.name + ", 文件名: " + file.size + ", 信息: " + message);
			break;
