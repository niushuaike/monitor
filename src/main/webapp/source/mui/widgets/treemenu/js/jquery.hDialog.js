...
;(function($, window, document, undefined) {	
	...
	methods = {
        ...
        open: function (o,T) {
			...
			w = o.width != undefined ? parseInt(o.width) : 300, h = o.height != undefined ? parseInt(o.height) : 270, m = ""+parseInt(-(h/2))+'px 0 0 '+parseInt(-(w/2))+"%";
			...
			//关闭按钮
			if(o.closeHide != false) closeBtnTpl = '<a id="HCloseBtn" title="关闭" style="width:50px;height:38px;line-height:50px;display:inline-block;cursor:pointer;background-color:'+closeBg+';color:#5a98de;text-align:center;position:absolute;top:5px;left:8px;"><span  style="width:24px;height:24px;display:inline-block;"><i class="fa fa-arrow-right fa-2x"></i></span></a>';
			...
			if(o.types == 2){ iframeTpl = '<iframe width="' + o.width + '" height="' + o.height + '" frameborder="0" scrolling="auto" src="' + o.iframeSrc + '"></iframe>'; $B.append('<div id="'+o.iframeId+'"></div>'); $obj = $('#'+o.iframeId); }
        		...
        		$B.stop().append(overlayTpl).find($obj).css({'background-color':'rgba(255,255,255,1)','position':'fixed','top':t,'left':l,'z-index':999999,'margin':m,'width':o.width,'height':"100%"}).removeAttr('class').addClass('animated '+c).prepend(headTpl+closeBtnTpl+iframeTpl).show();
        
        		//默认关闭
        		$close = $('#HCloseBtn');
        		if(o.modalHide) $close = $('#HCloseBtn');
	    ...
	    close: function (o,T) {
			...
			if($obj.hasClass(o.effect)){ setTimeout(function(){ $obj.removeAttr('style').hide(); },1000); }
